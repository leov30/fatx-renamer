...
for /f "delims=" %%g in ('dir /b /a:-d "%~1"') do set /a "_total_lines+=1"
...
echo -----------------------------------------------------
echo 1. Use short counry codes ^(USA^) ---^> ^(U^)
echo 2. Remove Parenthesis tags
echo 3. Do Nothing ^(Default^)
echo.

Set /p _option0=Enter The Number: || set _option0=nul

if %_option0%==1 set "_option0=short"
if %_option0%==2 (set "_option0=none") else (set _option0=nul) 
...
REM //remove parenthesis
if "%_option0%"=="none" for /f "tokens=1 delims=)(" %%h in ("%_name%") do set "_name=%%h"
...
if %3==0 %_option1% "%~1%2" "_REN\%_name%%2" >nul
if %3==1 %_option1% "%_folder%\%~1%2" "%_folder%_REN\%_name%%2" >nul

if %3==1 (echo "%~1%2 ---> %_name%%2") >>"%_folder%_REN.txt"

REM echo "%~1%2 ---> %_name%%2"
